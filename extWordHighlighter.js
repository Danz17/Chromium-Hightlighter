...
//var id = chrome.contextMenus.create({"title": "something", "id":"something", "contexts": ["selection"]});

function updateContextMenu(inUrl){
    debug&&console.log('updatig context menu', inUrl);
    if(inUrl&&noContextMenu.indexOf(inUrl)==-1){
        chrome.contextMenus.removeAll();
        createSearchMenu();
        var contexts = ["selection"];
        var filteredGroups=getWords(inUrl);
...
function getWords(inUrl){
    ...
    for (var highlightData in HighlightsData.Groups) {
        ...
        if (HighlightsData.Groups[highlightData].Enabled){
            ...
            for(var dontShowOn in HighlightsData.Groups[highlightData].DontShowOn){
